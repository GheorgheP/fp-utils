...
export function mCompose<A, R, F0 extends _F<A>>(fn: (a: A) => R, f0: F0): (...args: Parameters<F0>) => R | undefined;
export function mCompose<A, B, R, F0 extends _F<A>>(
  fn: (a: B) => R,
  f1: _F1<A, B>,
  f0: F0,
): (...args: Parameters<F0>) => R | undefined;
...
export function mCompose<F extends _Fn>(...fns: F[]): (...args: any) => any | undefined {
  let [f, ...fs] = fns.reverse();

  return (...v: _P<typeof f>): _Rt<typeof fns[0]> | undefined => {
    // @ts-ignore
    let _v = f(...v);
